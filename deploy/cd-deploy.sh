...
APP_DIR=/var/www/dojo
FETCH_URL=https://github.com/rigst/dojo.git   # HTTPS anônimo — repo público, sem credencial
VENV=/var/www/dojo/venv
ENV_FILE=/var/www/dojo/.env   # checkout direto — sem shared/
WEB_SERVICE=dojo.service      # reload (SIGHUP): zero downtime, socket nunca cai (ExecReload já configurado)
OTHER_SERVICES=()             # sem celery neste app
HEALTH_URL="https://dojo.stolben.com/saude/"
HEALTH_HEADER=""
BACKUP_SCRIPT=/var/www/dojo/deploy/backup_postgres.sh
EXTRA_ENV=""   # DJANGO_ENV=production já vem do .env; não precisa repetir aqui
LOCK_FILE=/tmp/dojo_cd_deploy.lock
...
main() {
  ...
  if [[ -n "$tem_migracao" && -n "$BACKUP_SCRIPT" ]]; then
    ...
    #   deploy ALL=(rod) NOPASSWD: /var/www/dojo/deploy/backup_postgres.sh
